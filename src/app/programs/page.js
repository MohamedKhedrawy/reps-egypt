    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <section className="pt-32 pb-12 px-6 border-b border-white/5">
        <div className="max-w-7xl mx-auto">
           ...
           <p className="text-gray-400 max-w-2xl mb-10 text-lg">World-class fitness education tailored for the Egyptian market. Start your professional journey today or level up your existing skills.</p>
           
           {/* Filters */}
           <div className="grid md:grid-cols-4 gap-4 p-4 bg-white/5 rounded-2xl border border-white/5">
              <input type="text" placeholder="Search programs..." className="bg-transparent border border-white/10 rounded-lg px-4 py-3 text-sm focus:border-red-600 focus:outline-none placeholder:text-gray-600" />
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
           {programs.map((program, idx) => (
             <div key={idx} className="group bg-[#121212] border border-white/5 rounded-2xl overflow-hidden hover:border-red-600/30 transition-all duration-500 flex flex-col shadow-2xl shadow-black/50">
               <div className="h-56 overflow-hidden relative">
                  <div className="absolute top-4 left-4 z-10 bg-red-600 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider shadow-lg">
                    {program.category}
                  </div>
               ...
               <div className="p-8 flex-grow flex flex-col">
                  <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-red-500 transition-colors">{program.title}</h3>
                  <p className="text-sm text-gray-400 mb-8 line-clamp-2 leading-relaxed">{program.desc}</p>
                  ...
                  <div className="mt-auto pt-6 border-t border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-xs font-semibold text-gray-300">By {program.instructor}</span>
                    </div>
                    <span className="text-xl font-bold text-white">{program.price}</span>
                  </div>
                  
                  <button className="mt-8 w-full py-3.5 bg-white/5 hover:bg-red-600 text-sm font-bold rounded-xl transition-all border border-white/5 hover:border-red-600 group-hover:shadow-[0_0_20px_rgba(220,38,38,0.2)]">
                     Enroll Now
                  </button>
               </div>
             </div>
        ...
        {/* Newsletter / CTA */}
        <div className="mt-32 p-12 bg-white/[0.02] border border-white/5 rounded-3xl text-center relative overflow-hidden">
           ...
           <p className="text-gray-400 mb-8 max-w-xl mx-auto">Subscribe to our newsletter to receive updates on new programs, workshops, and exclusive early-bird discounts.</p>
           
           <div className="flex flex-col sm:flex-row max-w-md mx-auto gap-3">
              <input type="email" placeholder="Email address" className="flex-1 bg-white/5 border border-white/10 rounded-xl px-5 py-3.5 text-sm focus:outline-none focus:border-red-600 transition-colors" />
              <button className="bg-white text-black font-extrabold px-8 py-3.5 rounded-xl hover:bg-gray-200 transition-colors">Join Now</button>
           </div>
        </div>
      </section>
