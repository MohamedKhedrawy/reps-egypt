...
export async function generateMetadata({ params }) {
    ...
    return {
        title: dictionary.standards_page.title_prefix + ' ' + dictionary.standards_page.title_highlight + ' | Reps Egypt',
        description: dictionary.standards_page.subtitle,
    };
}
...
export default async function StandardsPage({ params }) {
  ...
  const standards = dictionary.standards_page.standards_list.map((item, index) => ({
    ...item,
    icon: (
      // Icons mapping
      [
        ...
        <svg key={3} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 0 1-1.043 3.296 3.745 3.745 0 0 1-3.296 1.043A3.745 3.745 0 0 1 12 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 0 1-3.296-1.043 3.745 3.745 0 0 1-1.043-3.296A3.745 3.745 0 0 1 21 12Z" /></svg>,
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative py-24 px-6 flex flex-col items-center justify-center text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-red-900/10 border border-red-500/20 rounded-full mb-8">
            ...
            <span className="text-red-400 text-xs font-bold uppercase tracking-widest">{dictionary.standards_page.badge_text}</span>
          </div>
          
          <h1 className="text-4xl md:text-6xl font-bold mb-6 tracking-tight">
            {dictionary.standards_page.title_prefix} <span className="text-red-600">{dictionary.standards_page.title_highlight}</span>
          </h1>
          
          <p className="text-muted text-lg md:text-xl leading-relaxed max-w-2xl mx-auto">
            {dictionary.standards_page.subtitle}
          </p>
        </div>
      </section>
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
           {standards.map((item, idx) => (
             <div key={idx} className="bg-secondary border border-border p-8 rounded-2xl hover:border-red-600/30 hover:bg-tertiary transition-all group">
      ...
      <section className="py-20 bg-secondary border-t border-border">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-3xl font-bold mb-6 text-foreground">{dictionary.standards_page.docs_title}</h2>
          <p className="text-muted mb-8 max-w-2xl mx-auto">
            {dictionary.standards_page.docs_text}
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
             <button className="flex items-center gap-2 px-6 py-3 bg-background border border-border hover:bg-tertiary rounded-lg transition-colors text-sm font-medium text-foreground">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" />
                </svg>
                {dictionary.standards_page.btn_download_code}
             </button>
             <button className="flex items-center gap-2 px-6 py-3 bg-background border border-border hover:bg-tertiary rounded-lg transition-colors text-sm font-medium text-foreground">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                </svg>
                {dictionary.standards_page.btn_download_manual}
             </button>
          </div>
        </div>
      </section>
