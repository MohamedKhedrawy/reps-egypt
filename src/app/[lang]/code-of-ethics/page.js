...
export async function generateMetadata({ params }) {
    ...
    return {
        title: dictionary.ethics_page.title + ' | Reps Egypt',
        description: dictionary.ethics_page.subtitle,
    };
}

export default async function CodeOfEthicsPage({ params }) {
  const { lang } = await params;
  const dictionary = await getDictionary(lang);

  const principles = dictionary.ethics_page.principles.map((p, index) => ({
    ...p,
    icon: (
      // Icons corresponding to the order: Integrity, Welfare, Respect, Development
      [
        <svg key={0} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path fillRule="evenodd" d="M7.5 6a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM3.751 20.105a8.25 8.25 0 0 1 16.498 0 .75.75 0 0 1-.437.695A18.683 18.683 0 0 1 12 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 0 1-.437-.695Z" clipRule="evenodd" /></svg>,
        <svg key={1} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d="m11.645 20.91-.007-.003-.022-.012a15.247 15.247 0 0 1-.383-.218 25.18 25.18 0 0 1-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0 1 12 5.052 5.5 5.5 0 0 1 16.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 0 1-4.244 3.17 15.247 15.247 0 0 1-.383.219l-.022.012-.007.004-.003.001a.752.752 0 0 1-.704 0l-.003-.001Z" /></svg>,
        <svg key={2} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path fillRule="evenodd" d="M8.25 6.75a3.75 3.75 0 1 1 7.5 0 3.75 3.75 0 0 1-7.5 0ZM15.75 9.75a3 3 0 1 1 6 0 3 3 0 0 1-6 0ZM2.25 9.75a3 3 0 1 1 6 0 3 3 0 0 1-6 0ZM6.31 15.117A6.745 6.745 0 0 1 12 12a6.745 6.745 0 0 1 6.709 7.498.75.75 0 0 1-.372.636A16.657 16.657 0 0 1 12 21.75c-2.33 0-4.512-.645-6.374-1.766a.75.75 0 0 1-.371-.636ZM16.611 15.468C18.025 16.59 18.9 18.23 18.9 20.08a16.551 16.551 0 0 1-1.353 3.655.75.75 0 0 1-1.29-.696A15.061 15.061 0 0 1 17.391 20.08c0-1.285-.59-2.452-1.528-3.253a.75.75 0 1 1 .948-1.159ZM5.42 16.927a15.061 15.061 0 0 1 1.133 3.153.75.75 0 1 1-1.291.696 16.553 16.553 0 0 1-1.352-3.655c0-1.85.875-3.49 2.29-4.612a.75.75 0 0 1 .947 1.16Z" clipRule="evenodd" /></svg>,
        <svg key={3} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path fillRule="evenodd" d="M12 2.25a.75.75 0 0 1 .75.75v.756a14.053 14.053 0 0 1 5.96 2.01.75.75 0 0 1 .28.832 15.539 15.539 0 0 0 1.93 6.643c.25.42.067.97-.37 1.168a15.565 15.565 0 0 1-5.12 1.258V16.5a.75.75 0 0 1-.75.75h-2.25a.75.75 0 0 1-.75-.75v-1.12c-1.818-.242-3.568-.69-5.197-1.306a.75.75 0 0 1-.397-1.116 15.518 15.518 0 0 0 2.002-6.505.75.75 0 0 1 .28-.832A14.053 14.053 0 0 1 11.25 3.756V3a.75.75 0 0 1 .75-.75ZM12 21.75a2.25 2.25 0 0 1-2.25-2.25H12v2.25Z" clipRule="evenodd" /><path d="M12.75 2.308a15.548 15.548 0 0 1 5.034 1.838.75.75 0 0 1 .15 1.157 14.004 14.004 0 0 0-4.048 3.513.75.75 0 0 1-1.072 0 13.999 13.999 0 0 0-4.048-3.513.75.75 0 0 1 .15-1.157 15.548 15.548 0 0 1 5.034-1.838Z" /></svg>
      ][index]
    )
  }));
  ...
  const violationCategories = [
    {
      ...dictionary.ethics_page.violation_minor,
      color: "amber",
    ...
    {
      ...dictionary.ethics_page.violation_moderate,
      color: "orange",
      iconPath: "M9.401 3.003c1.155-2 4.043-2 5.197 0l7.355 12.748c1.154 2-.29 4.5-2.599 4.5H4.645c-2.309 0-3.752-2.5-2.598-4.5L9.4 3.003ZM12 8.25a.75.75 0 0 1 .75.75v3.75a.75.75 0 0 1-1.5 0V9a.75.75 0 0 1 .75-.75Zm0 8.25a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Z"
    },
    {
      ...dictionary.ethics_page.violation_severe,
      color: "red",
      iconPath: "M9.401 3.003c1.155-2 4.043-2 5.197 0l7.355 12.748c1.154 2-.29 4.5-2.599 4.5H4.645c-2.309 0-3.752-2.5-2.598-4.5L9.4 3.003ZM12 8.25a.75.75 0 0 1 .75.75v3.75a.75.75 0 0 1-1.5 0V9a.75.75 0 0 1 .75-.75Zm0 8.25a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Z"
    }
  ];
    ...
    <div className="min-h-screen bg-background text-foreground pt-12 pb-24">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl font-bold mb-3 text-foreground">{dictionary.ethics_page.title}</h1>
          <p className="text-muted">{dictionary.ethics_page.subtitle}</p>
        </div>

        {/* Commitment Card */}
        <div className="bg-secondary border border-border rounded-2xl p-8 mb-16">
           <h2 className="text-xl font-bold mb-4 text-foreground">{dictionary.ethics_page.commitment_title}</h2>
           <p className="text-muted text-sm leading-relaxed mb-4">
             {dictionary.ethics_page.commitment_text1}
           </p>
           <p className="text-muted text-sm leading-relaxed">
             {dictionary.ethics_page.commitment_text2}
           </p>
        </div>

        {/* Core Principles */}
        <div className="mb-20">
           <h2 className="text-2xl font-bold mb-8 text-center text-foreground">{dictionary.ethics_page.principles_title}</h2>
        ...
        {/* Violations Section */}
        <div className="mb-20">
           <h2 className="text-2xl font-bold mb-8 text-center text-foreground">{dictionary.ethics_page.violations_title}</h2>
              ...
              {violationCategories.map((v, idx) => {
                ...
                return (
                  <div key={idx} className={`bg-secondary border ${styles.border} rounded-2xl p-6`}>
                     ...
                     <p className="text-sm text-foreground/70 font-bold mb-2">{dictionary.ethics_page.violations_examples_label}</p>
                     <p className="text-sm text-muted mb-6">{v.examples}</p>
                     
                     <div className={`bg-tertiary p-4 rounded-xl border-l-4 ${styles.borderAccent}`}>
                        <p className={`text-xs font-bold ${styles.text} mb-1`}>{dictionary.ethics_page.violations_consequences_label}</p>
        ...
        <div className="bg-gradient-to-r from-secondary to-background border border-border rounded-2xl p-8 mb-16 relative overflow-hidden">
           ...
           <div className="relative z-10">
              <h2 className="text-xl font-bold mb-3 text-foreground">{dictionary.ethics_page.reporting_title}</h2>
        ...
        <div className="text-center">
           <p className="text-lg font-bold text-foreground mb-2">{dictionary.ethics_page.footer_title}</p>
           <p className="text-xs text-muted max-w-lg mx-auto mb-8">
             {dictionary.ethics_page.footer_text}
           </p>
           <p className="text-[10px] text-muted">{dictionary.ethics_page.updated_date}</p>
        </div>
