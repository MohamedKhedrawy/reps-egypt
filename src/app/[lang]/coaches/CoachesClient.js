...
export default function CoachesClient({ coaches, content, dictionary, lang }) {
  ...
  const filteredCoaches = useMemo(() => {
    return coaches.filter((coach) => {
      // Search by name
      const nameMatch = coach.fullName.toLowerCase().includes(searchQuery.toLowerCase());
      
      // Filter by specialty (normalize selected specialty to match database format)
      const normalizedSpecialty = selectedSpecialty.toLowerCase().replace(/['\s]/g, "-");
      const specialtyMatch = !selectedSpecialty || 
        (coach.specialization && coach.specialization === normalizedSpecialty);
      
      // Filter by experience
      const experienceMatch = !selectedExperience || 
        (coach.experience && coach.experience === selectedExperience);

      return nameMatch && specialtyMatch && experienceMatch;
    });
  }, [coaches, searchQuery, selectedSpecialty, selectedExperience]);
        ...
        <div className="max-w-7xl mx-auto">
           ...
           <div className="grid md:grid-cols-4 gap-4 p-4 bg-secondary rounded-2xl border border-border">
              <input 
                 ...
                 className="bg-background border border-border rounded-lg px-4 py-3 text-sm focus:border-red-600 focus:outline-none text-foreground placeholder-muted"
              />
              <div className="relative">
                <select 
                   value={selectedSpecialty}
                   onChange={(e) => setSelectedSpecialty(e.target.value)}
                   className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm focus:border-red-600 focus:outline-none appearance-none text-foreground"
                >
                   <option value="">{content.filter_specialty}</option>
                   {content.filter_specialty_options?.map((opt, i) => (
               ...
               <div className="relative">
                <select 
                   ...
                   className="w-full bg-background border border-border rounded-lg px-4 py-3 text-sm focus:border-red-600 focus:outline-none appearance-none text-foreground"
                >
                   <option value="">{content.filter_experience}</option>
                   {content.filter_experience_options?.map((opt, i) => (
                     <option key={i} value={opt}>{opt}</option>
                   ))}
                </select>
                 <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
              ...
              <button 
                 ...
                 className="bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg px-6 py-3 transition-colors"
              >
                {content.btn_search}
              </button>
           </div>
           ...
           {searchQuery || selectedSpecialty || selectedExperience ? (
             <p className="text-sm text-muted mt-4">
               Found <span className="font-bold text-foreground">{filteredCoaches.length}</span> {filteredCoaches.length === 1 ? 'coach' : 'coaches'}
             </p>
           ) : null}
        </div>
      </section>
         ...
         {filteredCoaches.length === 0 ? (
             <div className="text-center py-20 text-muted">
                 <h3 className="text-2xl font-bold mb-2">
                   {searchQuery || selectedSpecialty || selectedExperience 
                     ? content.no_results_title || 'No coaches found' 
                     : content.no_coaches_title}
                 </h3>
                 <p>
                   {searchQuery || selectedSpecialty || selectedExperience 
                     ? content.no_results_desc || 'Try adjusting your filters' 
                     : content.no_coaches_desc}
                 </p>
             </div>
         ) : (
             <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                {filteredCoaches.map((trainer) => (
                  <div key={trainer._id.toString()} className="group relative bg-secondary rounded-xl overflow-hidden border border-border hover:border-red-600/50 transition-all duration-300 shadow-xl shadow-black/10 hover:shadow-red-600/5">
                    <div className="aspect-[4/5] w-full overflow-hidden relative">
                       {/* Badge */}
                       <div suppressHydrationWarning={true} className="absolute top-3 left-3 z-10 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded border border-white/10 flex items-center gap-1">
