            ...
            <div className="lg:col-span-2 space-y-8">
                ...
                {activeTab === "Membership" && (
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-secondary border border-border rounded-2xl p-8 relative overflow-hidden">
                      {/* Stylized Background */}
                      <div className="absolute top-0 right-0 w-64 h-64 bg-red-600/5 blur-[100px] -z-10" />

                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div>
                          <h2 className="text-sm font-bold text-muted uppercase tracking-wider mb-2">Current Plan</h2>
                          <div className="flex items-center gap-3">
                            <span className="text-3xl font-bold text-foreground">
                              {user.role === 'trainer' && user.repsId ? "Pro Member" : "Student"}
                            </span>
                            {user.role === 'trainer' && user.repsId && (
                              <span className="px-3 py-1 bg-red-600/10 text-red-500 text-xs font-bold rounded-full border border-red-600/20">
                                ACTIVE
                              </span>
                            )}
                          </div>
                          <p className="text-sm text-muted mt-2 max-w-md">
                            {user.role === 'trainer' && user.repsId 
                              ? "You have full access to all professional features including liability insurance and job board."
                              : "Upgrade to a professional membership to unlock certification, insurance, and more."}
                          </p>
                        </div>

                        <Link 
                          href="/pricing"
                          className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl transition-all shadow-[0_0_20px_rgba(220,38,38,0.3)] hover:shadow-[0_0_30px_rgba(220,38,38,0.5)] whitespace-nowrap"
                        >
                          {user.role === 'trainer' && user.repsId ? "Manage Subscription" : "Upgrade Plan"}
                        </Link>
                      </div>
                      ...
                      <div className="mt-8 pt-8 border-t border-border grid grid-cols-2 md:grid-cols-4 gap-4">
                         {[
                           { label: "Profile Status", value: "Public" },
                           { label: "Insurance", value: user.role === 'trainer' && user.repsId ? "Active" : "Inactive" },
                           { label: "Job Board", value: user.role === 'trainer' && user.repsId ? "Unlimited" : "Locked" },
                           { label: "Next Billing", value: user.role === 'trainer' && user.repsId ? "Dec 31, 2024" : "--" },
                         ].map((stat, i) => (
                           <div key={i}>
                             <div className="text-xs text-muted mb-1">{stat.label}</div>
                             <div className={`font-bold ${stat.value === 'Inactive' || stat.value === 'Locked' ? 'text-muted' : 'text-foreground'}`}>{stat.value}</div>
                           </div>
                         ))}
                      </div>
                    </div>
