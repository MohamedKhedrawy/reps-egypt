...
export default function TrainerRegisterClient({ content, lang }) {
  ...
  const validators = {
    ...
    age: (v) => !v ? "" : parseInt(v) < 18 ? content.errors.age_min : parseInt(v) > 80 ? content.errors.age_max : "",
    specialization: (v) => !v ? content.errors.first_name_required : "",
  ...
  const stepTitles = {
    ...
    3: { title: content.step3_title, subtitle: content.step3_subtitle }
  };
        ...
        <div className="bg-secondary border border-border rounded-3xl p-8 md:p-10 shadow-2xl">
          ...
          {currentStep === 3 && (
            <form onSubmit={handleSubmit} className="space-y-8">
              <div><label className="text-xs font-bold text-muted uppercase tracking-wider">{content.specialization} *</label>
                <select name="specialization" value={formData.specialization} onChange={handleChange} className={`${inputClass("specialization")} appearance-none`}>
                  <option value="">{content.specialization_placeholder}</option>
                  {content.specialization_options.map((o, i) => <option key={i} value={o.toLowerCase().replace(/['\s]/g, "-")}>{o}</option>)}
                </select>
                <FieldError name="specialization" />
              </div>
              <div><label className="text-xs font-bold text-muted uppercase tracking-wider">{content.upload_certs}</label>
                <div className="relative"><input type="file" multiple ref={certFileInputRef} onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                  <div className="border-2 border-dashed border-border rounded-xl p-8 text-center hover:border-red-600/50 transition-colors">
                    ...
                    <p className="text-sm text-muted">{content.upload_drop} <span className="text-red-500 font-semibold">{content.upload_browse}</span></p>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-4 pt-4">
                <button type="button" onClick={() => setCurrentStep(2)} className="w-full md:w-auto px-8 py-3.5 bg-transparent border border-border hover:bg-tertiary text-foreground font-bold rounded-xl transition-colors">{content.btn_back}</button>
